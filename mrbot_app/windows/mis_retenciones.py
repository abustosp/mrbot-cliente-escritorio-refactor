...
class MisRetencionesWindow(BaseWindow):
    ...
    def __init__(self, master=None, config_provider=None, example_paths: Optional[Dict[str, str]] = None):
        ...
        self.ret_df: Optional[pd.DataFrame] = None

        container = ttk.Frame(self, padding=10)
        container.pack(fill="both", expand=True)
        self.add_section_label(container, "Mis Retenciones")
        self.add_info_label(
            container,
        ...
        ttk.Button(btns, text="Previsualizar Excel", command=lambda: self.open_df_preview(self._filter_procesar(self.ret_df), "Previsualizacion Mis Retenciones")).grid(
        ...
        self.log_text = self.add_collapsible_log(container, title="Logs de ejecucion", height=10, service="mis_retenciones")
    ...
    def abrir_ejemplo(self) -> None:
        path = self.example_paths.get("mis_retenciones.xlsx")
        if not path:
    ...
    def cargar_excel(self) -> None:
        ...
        if not filename:
            return
        try:
            self.ret_df = pd.read_excel(filename, dtype=str).fillna("")
            self.ret_df.columns = [c.strip().lower() for c in self.ret_df.columns]
            df_prev = self._filter_procesar(self.ret_df)
            self.set_preview(self.preview, df_preview(df_prev if df_prev is not None else self.ret_df))
        ...
        if isinstance(data, dict):
            ...
            if isinstance(archivos, list):
                for idx, item in enumerate(archivos, start=1):
                    ...
                    name_hint = item.get("archivo") or item.get("tipo") or item.get("impuesto_label")
                    link = build_link(url, name_hint, "retencion", idx)
                    if link:
                        ...
                        if key not in seen:
                            ...
                            links.append(link)
        if not links:
            links = collect_minio_links(data, "retencion")
        return links
    ...
    def procesar_excel(self) -> None:
        if self.ret_df is None or self.ret_df.empty:
        ...
        df_to_process = self._filter_procesar(self.ret_df)
