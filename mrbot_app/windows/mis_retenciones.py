...
class MisRetencionesWindow(BaseWindow, ExcelHandlerMixin, DateRangeHandlerMixin, DownloadHandlerMixin):
    ...
    def __init__(self, master=None, config_provider=None, example_paths: Optional[Dict[str, str]] = None):
        ...
        ttk.Button(btns, text="Ejemplo Excel", command=lambda: self.abrir_ejemplo_key("mis_retenciones.xlsx")).grid(row=0, column=2, padx=4, pady=2, sticky="ew")
        ttk.Button(btns, text="Previsualizar Excel", command=lambda: self.previsualizar_excel("Previsualizacion Mis Retenciones")).grid(
    ...
    def _process_row_retenciones(self, row, url, headers, default_desde, default_hasta):
        ...
        downloads, errors, download_dir = self._process_downloads(
            data, self.MODULE_DIR, cuit_folder, override_dir=row_download, service_key="retencion"
        )
