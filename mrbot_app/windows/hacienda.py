...
class HaciendaWindow(BaseWindow, ExcelHandlerMixin, DateRangeHandlerMixin, DownloadHandlerMixin):
    MODULE_DIR = "Hacienda"

    def __init__(self, master=None, config_provider=None, example_paths: Optional[Dict[str, str]] = None):
        super().__init__(master, title="Liquidaciones Hacienda y Carne", config_provider=config_provider)
        ...
        self.add_section_label(container, "Consulta de Hacienda y Carne")
        self.add_info_label(
            container,
            "Consulta individual o masiva por Excel para /api/v1/hacienda/consulta. "
            "Debe incluir cuit_representante, denominacion, representado_cuit y clave. "
            "minio_upload se envia siempre en True para descargar archivos.",
        )
        ...
        ttk.Label(inputs, text="Denominacion").grid(row=1, column=0, sticky="w", padx=4, pady=2)
        ...
        ttk.Entry(inputs, textvariable=self.denominacion_var, width=25).grid(row=1, column=1, padx=4, pady=2, sticky="ew")
        ...
        ttk.Button(btns, text="Ejemplo Excel", command=lambda: self.abrir_ejemplo_key("hacienda.xlsx")).grid(row=0, column=2, padx=4, pady=2, sticky="ew")
        ttk.Button(btns, text="Previsualizar Excel", command=lambda: self.previsualizar_excel("Previsualizacion Hacienda")).grid(
        ...
        self.log_text = self.add_collapsible_log(container, title="Logs de ejecucion", height=10, service="hacienda")
        ...
        if isinstance(data, dict):
            ...
            if isinstance(comprobantes, list):
                for idx, item in enumerate(comprobantes, start=1):
                    ...
                    name_hint = item.get("archivo") or item.get("consulta")
                    link = build_link(url, name_hint, "hacienda", idx)
                    if link:
                        ...
                        if key not in seen:
                            ...
                            links.append(link)
        if not links:
            links = collect_minio_links(data, "hacienda")
        return links
    ...
    def _worker_excel(self, df, url, headers, default_desde, default_hasta, default_proxy):
        ...
        self.set_execution_summary(self.build_download_execution_summary("Hacienda", rows, total_expected=total))
